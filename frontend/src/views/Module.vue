<template>
  ...
  <div class="w-full self-center bg-purple-200 my-6" style="height: 1px;"></div>
    ...
    <h1 class="font-display text-2xl text-secondary font-medium px-4">Enrollment Status</h1>
    <div class="bg-white w-full shadow-sm rounded-md flex flex-col justify-start items-start px-6 py-4 mt-4">
      ...
      <h3 v-if="existingCohort.status == 0" class="font-body text-sm mt-2">{{ existingCohort.learner_count }}/15 learners enrolled <span class="text-primary">( {{ Math.round(existingCohort.learner_count/15 * 100)/100 }}% )</span></h3>
    ...
    <div class="w-full" v-if="!enrolled">
    <h1 v-if="showAvailability && !loading" class="font-display text-2xl text-secondary font-medium px-4">Available Cohorts</h1>
...
import { getAvailableCohorts, joinCohort, getModuleCohort, leaveModuleCohort } from '../services/CohortService'
  ...
  created() {
    ...
    firebase.auth().onAuthStateChanged(async (user) => {
      if (user) {
        ...
        this.existingCohort = await getModuleCohort(this.token, this.module.id)
